...
export const Home = () => {
  ...
  const [stakingStats, totalSupply, priceU2u] = useStakingStore(state => [
    state.stakingStats,
    state.totalSupply,
    state.priceU2u,
  ])
  ...
  const marketCap = useMemo(() => {
    try {
      return Number(Number(totalSupply)) * Number(priceU2u)
    } catch (error) {
      return 0
    }
  }, [totalSupply, priceU2u]) // TODO: calculate market cap

  const stakedValue = useMemo(() => {
    try {
      return Number(bigFormatEther(totalStaked)) * Number(priceU2u)
    } catch (error) {
      return 0
    }
  }, [totalStaked, priceU2u]) // TODO: calculate market cap
            ...
            <div className="w-full py-3 flex justify-around flex-wrap gap-6">
              ...
              <div className="">
                ...
                <div className="text-lg text-neutral font-semibold">
                  <RenderNumberFormat amount={epochRewards ? bigFormatEther(epochRewards) : 0} className="mr-2"
                                      fractionDigits={2}/>
                </div>
              </div>
              <div className="">
              ...
              <div className="">
                ...
                <div className="text-lg text-neutral font-semibold">{shortenDisplayNumber(marketCap)}</div>
              </div>
              <div className="">
                <div className="text-xs text-neutral">{t('Staked value (USD)')}</div>
                <div className="text-lg text-neutral font-semibold">
                  {shortenDisplayNumber(stakedValue)}
                </div>
              </div>
              <div className="">
                <div className="text-xs text-neutral">{t('Price(USD)')}</div>
                <div className="text-lg text-neutral font-semibold">
                  {Number(priceU2u).toFixed(5)}
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="pt-6">
        <div className="bg-gradient-primary">
          <div className="w-full py-3 px-[41px] flex justify-around flex-wrap gap-6">
            ...
            <div className="">
              <div className="text-base text-neutral font-semibold">{t('Price (USD)')}</div>
            ...
            <div className="">
              ...
              <div className="text-2xl text-neutral font-bold">{shortenDisplayNumber(marketCap)}</div>
            </div>
            <div className="">
              <div className="ext-base text-neutral font-semibold">{t('Staked value (USD)')}</div>
              <div className="text-2xl text-neutral font-bold">
                <div className="text-2xl text-neutral font-bold">{shortenDisplayNumber(stakedValue)}</div>
              </div>
            </div>
          </div>
        </div>
