    ...
    <div className="w-full overflow-x-auto">
      ...
      <thead>
          <tr className="border-y border-border-outline text-text-secondary font-medium">
            <th className="py-6 text-left px-6 font-medium whitespace-nowrap">{t("Epoch")}</th>
            <th className="py-6 text-right font-medium whitespace-nowrap">{t("Rewards (U2U)")}</th>
            <th className="py-6 text-right font-medium px-6 whitespace-nowrap">{t("End Time")}</th>
          </tr>
        </thead>
        <tbody>
          {
            epoches && epoches.length > 0 ? epoches.map((row: ValidatorEpochInfo, index: number) => {
              return (
                <tr key={index} className="border-y border-border-outline font-semibold hover:bg-neutral-surface-hover cursor-pointer">
                  ...
                  <td className="text-base font-semibold text-text py-4 text-right whitespace-nowrap">
                    <RenderNumberFormat amount={bigFormatEther(row.epochRewards)} fractionDigits={2} />
                  </td>
                  <td className="text-base font-semibold text-text py-4 text-right px-6 whitespace-nowrap">
